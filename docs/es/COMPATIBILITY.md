...
| `AutoMapper@16.2.0` | 95.5% (2.319 métodos, 104 marcados) | La Fase 3.64 confirmó que la propia generación pesada de planes de mapeo basada en `System.Linq.Expressions` (~300 hallazgos de árbol de expresión, dominando todo lo demás) es un compilador general real de árbol de expresión a ejecutable que este proyecto no tiene — una empresa sustancialmente más grande que el propio `Expression.Compile()` angosto y solo-cadena-de-acceso-a-propiedad que la Fase 3.64 agregó para `FluentValidation`. Tratá este número como más optimista que los demás de esta tabla hasta que eso llegue. |
