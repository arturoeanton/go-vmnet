...
| `FluentValidation@11.9.2` | 98.0% (1,289 methods, 26 flagged) | [`examples/fluentvalidation-demo`](../../examples/fluentvalidation-demo) | **Verified for real object validation** — a real validator (`RuleFor`/`NotEmpty`/`WithMessage`) both accepts a valid object and rejects an invalid one with the correct error message. Getting here needed `Expression<TDelegate>.Compile()` to genuinely work for a real (if narrow) class of expression trees (Fase 3.64) — FluentValidation compiles and invokes the property-access lambda to read the value being validated, not just inspects its shape. **Known, real, separate limitation, precisely diagnosed in Fase 3.66** (correcting Fase 3.64's own guess): numeric range validators (`GreaterThanOrEqualTo`, etc.) receive a real `ValidationContext` object where the actual property value should be, and the real property value where the comparison constant should be — traced to two same-named, same-arity `IsValid` overloads across a generic base/derived class pair, not `Comparer<T>.Default` caching (that native is stateless, confirmed) — root cause not yet found. The demo deliberately only exercises the string validators that already work correctly. |
