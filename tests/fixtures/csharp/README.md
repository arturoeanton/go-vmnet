...
| `CheapWins3.cs` | `CheapWins3` | `NotImplementedException`, `Double.IsInfinity`/`IsPositiveInfinity`/`IsNegativeInfinity`, `String.EndsWith`, `Math.Floor`, `List.Clear`, `Int32.Parse`/`TryParse`/`CompareTo`, `Dictionary.Remove`, `DateTime.Kind`, `KeyValuePair<K,V>` direct-local ctor, `IList<T>`/`IReadOnlyCollection<T>` por interfaz | Fase 3.21 |
| `Async.cs` | `AsyncTest` | `async`/`await` (dos awaits secuenciales, excepción tras un await propagando a un catch síncrono, método `async Task` void, cadena de `await` anidada) | Fase 3.22 |
| `CheapWins4.cs` | `CheapWins4` | `DateTimeOffset`, `DateTime` operadores (`-`, `==`), `Double.TryParse`, `Convert.ToInt64`, `Char.ToLowerInvariant`, `Int64.ToString`, `ValueTuple` (campo sin puntero administrado), más LINQ (`SelectMany`/`Contains`/`Take`), `CultureInfo`, `IList` (firma distinta a la interfaz redirigida) | Fase 3.23 |
